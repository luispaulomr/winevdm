...
*/
	...
	static int _read_shared_data(void **msg, size_t len_msg)
	{
		HANDLE hMapFile;
		struct vm86_opt *tmp = NULL;
		...
		*msg = MapViewOfFile(hMapFile, // handle to map object
			FILE_MAP_READ,
			0,
			0,
			len_msg);
	...
	struct _aob {
		UINT8 aob[1024];        /* string to search */
		UINT32 i_aob;           /* current index */
		UINT32 i_max;           /* max index */
	...
	static BOOL _scan_memory_for_aob(UINT32 mem_addr, char* str)
	{
		unsigned char aob[255] = { 0 };
		int str_len = strlen(str) / 2;

		for (size_t i = 0; i < str_len; ++i) {
			(void)sscanf(str, "%2hhx", &aob[i]);
			str += 2;

			if (read_byte(mem_addr + i) != aob[i]) {
				return false;
			}
		}
	...
	static BOOL _aob_scan_check(struct _aob* _aob, UINT8* oprom, int opsize)
	{
		if (memcmp(&_aob->aob[_aob->i_aob], oprom, opsize)) {
			return false;
		}
		_aob->i_aob += opsize;
		return true;
	...
	static UINT8 _get_hex_from_str(UINT8* aob, const char* str)
	{
		int str_len = strlen(str) / 2;
	...
	static void _aob_clear(struct _aob* _aob)
	{
		_aob->i_aob = 0;
		_aob->i_print = 0;
		_aob->prev_buf[0] = 0;
	...
	(
		struct _aob* _aob,		/* aob struct */
		const char* aob_str,	/* AOB string to search */
		UINT8* oprom,			/* hex opcodes buf */
		int opsize,				/* buf size */
		char* buf				/* mnemonics buf */
	)
	{
		/* 
		 * convert AOB string to AOB hex buffer, 
		 * for comparison purposes, and clear
		 * AOB struct data
		 */
		if (!_aob->i_max) {
			_aob->i_max = _get_hex_from_str(_aob->aob, aob_str);
			_aob_clear(_aob);
		}

		/* compare AOB buffer with opcodes buf */
		if (_aob_scan_check(_aob, oprom, opsize)) {
			UINT32* i_print = &_aob->i_print;

			*i_print += sprintf(&_aob->to_print[*i_print], "AOB\n");

			/* print previous memory location */

			UINT32 mem_addr = _get_mem_address(_aob->prev_buf);
			if (mem_addr) {
				/* print memory at AOB memory */
				for (int i = 0; i < _aob->prev_opsize; ++i) {
			...
			strncpy(_aob->prev_buf, buf, sizeof(_aob->prev_buf));
			strncpy((char*)_aob->prev_oprom, (char*)oprom, opsize);
			_aob->prev_opsize = opsize;

			_aob->to_print[*i_print] = '\0';

			if (_aob->i_aob >= _aob->i_max) {
				/* AOB buffer was found, print data */
				fprintf(stderr, _aob->to_print);
				_aob_clear(_aob);
			}
...
__except (EXCEPTION_EXECUTE_HANDLER) {
		...
		if (dasm
			...
			if (!p_vm86_opts) {
				_read_shared_data(&p_shared_data, sizeof(struct vm86_opt));
				p_vm86_opts = (struct vm86_opt*) p_shared_data;
			}
